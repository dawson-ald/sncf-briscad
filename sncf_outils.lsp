...
(defun SIA:RunVectorizer (exePath imagePath combinationsArg / tmpDir batPath lines shell ret)
  ;; Passe par un fichier BAT pour accepter les chemins avec espaces.
  ;;
  ;; Exemple :
  ;; "C:\Users\...\Documents\BricsCAD\vectorizer.exe" "C:\Chemin\Image 1.png" "0, 3"
...
(defun apply-image-transparency (ent /)
  (if ent
    (progn
      ;; TRANSPARENCY fonctionne sur les images PNG/TIFF avec alpha,
      ;; et sur certains bitonaux. On l'active automatiquement.
...
;; Largeur visuelle approx. plus fiable que strlen seul.
;; Les anciens calculs sous-estimaient surtout Designation / Calque avec
;; majuscules, tirets, underscores et noms de calques longs.
...
;; Si le texte est trop long, il passe en alignement FIT entre xL et xR :
;; il reste donc dans la cellule au lieu de depasser du tableau.
...
(defun stab_make_block_def
  ...
  ;; Augmente un peu si tu veux que les cellules longues soient moins compressees.
  (setq charW 0.125)
  (setq padding 0.55)
  ...
  ;; Designation et Calque ont des minimums plus larges car ce sont les champs
  ;; qui contiennent souvent des textes longs.
