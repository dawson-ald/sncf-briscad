...
; IMPORTANT :
; - Ne jamais changer cet identifiant dans les futures versions
; - Il sert a retrouver le fichier .lsp actuel automatiquement
(setq *S_UPDATE_UNIQUE_ID* "SNCF_BRICSCAD_AUTOUPDATE_2026_06_12")

; URL RAW GitHub du fichier version.txt
; Le fichier doit contenir uniquement la version, exemple :
; 1.0.1
(setq *S_UPDATE_VERSION_URL*
  "https://raw.githubusercontent.com/dawson-ald/sncf-briscad/refs/heads/main/version.txt"
)

(setq *S_UPDATE_LSP_URL*
  "https://raw.githubusercontent.com/dawson-ald/sncf-briscad/refs/heads/main/sncf_outils.lsp"
)

; Capture native du chemin du LISP charge
; Peut etre NIL selon BricsCAD
(setq *S_UPDATE_CURRENT_LSP_PATH*
...
(defun s:cmd-quote (txt /)
  (strcat "\"" txt "\"")
)

(defun s:get-documents-folder (/ sh folders path)
  (setq path nil)
  ...
  (if sh
    (progn
      ...
      (setq path (vlax-invoke-method folders 'Item "MyDocuments"))
...
(defun s:ensure-folder (folder / parent)
  (if (and folder (/= folder "") (not (vl-file-directory-p folder)))
    (progn
      (setq parent (vl-filename-directory folder))

      (if (and parent (/= parent "") (not (vl-file-directory-p parent)))
        (s:ensure-folder parent)
      )
...
(defun s:get-temp-folder (/ docs folder)
  (setq docs (s:get-documents-folder))

  (if (not docs)
    (setq docs (getvar "TEMPPREFIX"))
  )

  (setq folder (strcat docs "\\BricsCAD\\updates"))
  (s:ensure-folder folder)
  folder
)

(defun s:get-temp-file (name /)
  (strcat (s:get-temp-folder) "\\" name)
)

(defun s:write-file-lines (file lines / f)
  (setq f (open file "w"))
...
(defun s:read-file-all (file / f line content)
  (setq content "")
...
(defun s:update-ps-file (/)
  (s:get-temp-file "update_background.ps1")
)

(defun s:update-status-file (/)
  (s:get-temp-file "update_status.txt")
)

(defun s:update-log-file (/)
  (s:get-temp-file "update_log.txt")
)

(defun s:update-new-lisp-file (/)
  (s:get-temp-file "github_new_script.lsp")
)

(defun s:update-version-file (/)
  (s:get-temp-file "github_version.txt")
)

; ============================================================
; CREATION DU SCRIPT POWERSHELL ARRIERE-PLAN
; ============================================================

(defun s:create-background-update-ps (/ psFile statusFile logFile newLispFile versionFile dwgFolder nativePath)
  (setq psFile (s:update-ps-file))
  (setq statusFile (s:update-status-file))
  (setq logFile (s:update-log-file))
  (setq newLispFile (s:update-new-lisp-file))
  (setq versionFile (s:update-version-file))
  (setq dwgFolder (getvar "DWGPREFIX"))

  (setq nativePath
    (if (and *S_UPDATE_CURRENT_LSP_PATH* (findfile *S_UPDATE_CURRENT_LSP_PATH*))
      *S_UPDATE_CURRENT_LSP_PATH*
      ""
    )
  )
  ...
  (s:write-file-lines
    ...
    (list
      ...
      (strcat "$marker = '" (s:ps-escape *S_UPDATE_UNIQUE_ID*) "'")
      (strcat "$currentVersion = '" (s:ps-escape *S_SCRIPT_VERSION*) "'")
      (strcat "$versionUrl = '" (s:ps-escape *S_UPDATE_VERSION_URL*) "'")
      (strcat "$lspUrl = '" (s:ps-escape *S_UPDATE_LSP_URL*) "'")
      (strcat "$statusFile = '" (s:ps-escape statusFile) "'")
      (strcat "$logFile = '" (s:ps-escape logFile) "'")
      (strcat "$newLspFile = '" (s:ps-escape newLispFile) "'")
      (strcat "$versionFile = '" (s:ps-escape versionFile) "'")
      (strcat "$nativePath = '" (s:ps-escape nativePath) "'")
      (strcat "$dwgFolder = '" (s:ps-escape dwgFolder) "'")

      "function Write-Status($state, $message, $path, $remoteVersion) {"
      "  $lines = @()"
      "  $lines += 'STATE=' + $state"
      "  $lines += 'MESSAGE=' + $message"
      "  $lines += 'PATH=' + $path"
      "  $lines += 'CURRENT_VERSION=' + $currentVersion"
      "  $lines += 'REMOTE_VERSION=' + $remoteVersion"
      "  $lines += 'DATE=' + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
      "  Set-Content -Path $statusFile -Value $lines -Encoding UTF8"
      "}"

      "function Log($txt) {"
      "  Add-Content -Path $logFile -Value ((Get-Date -Format 'HH:mm:ss') + ' - ' + $txt) -Encoding UTF8"
      "}"

      "function Download-NoCache($url, $outFile) {"
      "  $guid = [guid]::NewGuid().ToString()"
      "  if ($url.Contains('?')) {"
      "    $finalUrl = $url + '&nocache=' + $guid"
      "  } else {"
      "    $finalUrl = $url + '?nocache=' + $guid"
      "  }"
      "  Log ('Telechargement : ' + $finalUrl)"
      "  Invoke-WebRequest -Uri $finalUrl -OutFile $outFile -UseBasicParsing -Headers @{ 'Cache-Control'='no-cache'; 'Pragma'='no-cache' }"
      "}"

      "try {"
      "  Write-Status 'RUNNING' 'Verification en cours...' '' ''"
      "  Log 'Debut verification'"

      "  [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12"

      "  $currentLsp = ''"

      "  if ($nativePath -and (Test-Path $nativePath)) {"
      "    $currentLsp = $nativePath"
      "    Log ('Chemin natif utilise : ' + $currentLsp)"
      "  }"

      "  if (-not $currentLsp) {"
      "    Log 'Recherche du fichier LISP par identifiant unique'"

      "    $roots = @()"
      "    $roots += $dwgFolder"
      "    $roots += [Environment]::GetFolderPath('MyDocuments')"
      "    $roots += [Environment]::GetFolderPath('Desktop')"
      "    $roots += Join-Path $env:USERPROFILE 'OneDrive - SNCF'"
      ...
      "    $limitDate = (Get-Date).AddDays(-120)"
      "    $matches = @()"

      "    foreach ($root in $roots) {"
      "      try {"
      "        Log ('Recherche dans : ' + $root)"
      "        $files = Get-ChildItem -Path $root -Filter '*.lsp' -File -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.LastWriteTime -ge $limitDate }"
      "        foreach ($file in $files) {"
      "          try {"
      "            $content = Get-Content -Path $file.FullName -Raw -ErrorAction Stop"
      "            if ($content -like \"*$marker*\") {"
      "              $matches += $file"
      "              Log ('Match : ' + $file.FullName)"
      "            }"
      "          } catch {}"
      "        }"
      "      } catch {}"
      ...
      "    if ($best) {"
      "      $currentLsp = $best.FullName"
      "      Log ('Fichier retenu : ' + $currentLsp)"
      "    }"
      "  }"

      "  if (-not $currentLsp) {"
      "    Write-Status 'ERROR' 'Impossible de trouver le fichier LISP actuel.' '' ''"
      "    Log 'Erreur : fichier LISP actuel introuvable'"
      "    exit"
      "  }"

      "  if (Test-Path $versionFile) { Remove-Item $versionFile -Force }"
      "  if (Test-Path $newLspFile) { Remove-Item $newLspFile -Force }"

      "  Log 'Telechargement du fichier version GitHub sans cache'"
      "  Download-NoCache $versionUrl $versionFile"

      "  if (-not (Test-Path $versionFile)) {"
      "    Write-Status 'ERROR' 'Le fichier version GitHub n a pas ete telecharge.' $currentLsp ''"
      "    Log 'Erreur : version.txt introuvable apres telechargement'"
      "    exit"
      "  }"

      "  $remoteVersion = (Get-Content -Path $versionFile -Raw).Trim()"

      "  Log ('Version locale : ' + $currentVersion)"
      "  Log ('Version GitHub recue : ' + $remoteVersion)"

      "  if (-not $remoteVersion) {"
      "    Write-Status 'ERROR' 'Le fichier version GitHub est vide.' $currentLsp ''"
      "    Log 'Erreur : version distante vide'"
      "    exit"
      "  }"

      "  if ($remoteVersion -eq $currentVersion) {"
      "    Write-Status 'NO_UPDATE' 'Le script est deja a jour.' $currentLsp $remoteVersion"
      "    Log 'Aucune mise a jour necessaire'"
      "    exit"
      "  }"

      "  Log 'Nouvelle version detectee'"
      "  Log 'Telechargement du nouveau fichier LISP sans cache'"
      "  Download-NoCache $lspUrl $newLspFile"

      "  if (-not (Test-Path $newLspFile)) {"
      "    Write-Status 'ERROR' 'Le nouveau fichier LISP n a pas ete telecharge.' $currentLsp $remoteVersion"
      "    Log 'Erreur : nouveau LISP introuvable apres telechargement'"
      "    exit"
      "  }"

      "  $newContent = (Get-Content -Path $newLspFile -Raw)"

      "  if (-not $newContent.Trim()) {"
      "    Write-Status 'ERROR' 'Le nouveau fichier LISP telecharge est vide.' $currentLsp $remoteVersion"
      "    Log 'Erreur : nouveau LISP vide'"
      "    exit"
      "  }"

      "  if ($newContent -notlike \"*$marker*\") {"
      "    Write-Status 'ERROR' 'Le nouveau fichier LISP ne contient pas l identifiant unique.' $currentLsp $remoteVersion"
      "    Log 'Erreur : identifiant unique absent du nouveau fichier'"
      "    exit"
      "  }"

      "  if ($newContent -notlike \"*$remoteVersion*\") {"
      "    Log 'Attention : la version distante n apparait pas clairement dans le nouveau LISP'"
      "  }"

      "  $dir = Split-Path $currentLsp -Parent"
      "  $base = [System.IO.Path]::GetFileNameWithoutExtension($currentLsp)"
      "  $ext = [System.IO.Path]::GetExtension($currentLsp)"
      "  $date = Get-Date -Format 'yyyyMMdd_HHmmss'"
      "  $backup = Join-Path $dir ($base + '_backup_' + $date + $ext)"

      "  Log ('Creation backup : ' + $backup)"
      "  Copy-Item -Path $currentLsp -Destination $backup -Force"

      "  Log 'Remplacement du fichier LISP actuel'"
      "  Copy-Item -Path $newLspFile -Destination $currentLsp -Force"

      "  Write-Status 'UPDATED' 'Mise a jour terminee. Recharge le LISP ou redemarre BricsCAD.' $currentLsp $remoteVersion"
...
; ============================================================
; LANCEMENT POWERSHELL NON BLOQUANT
; ============================================================

(defun s:run-powershell-background (psFile / sh cmd result)
  (setq sh (vlax-create-object "WScript.Shell"))
  ...
  (if (not sh)
    ...
    (progn
      ...
      ; 0 = fenetre cachee
      ; :vlax-false = ne pas attendre la fin
      ; BricsCAD reste utilisable
      (setq result
        (vl-catch-all-apply
          'vlax-invoke-method
          (list sh 'Run cmd 0 :vlax-false)
        )
      )
...
(defun s:string-split-lines (txt / lines pos line)
  (setq lines nil)
...
(defun s:get-status-value (key / content lines prefix value len)
  (setq content (s:read-file-all (s:update-status-file)))
  (setq lines (s:string-split-lines content))
  (setq prefix (strcat key "="))
  (setq len (strlen prefix))
  (setq value "")

  (foreach line lines
    (if (and (>= (strlen line) len) (= (strcase (substr line 1 len)) (strcase prefix)))
      (setq value (substr line (+ len 1)))
    )
  )
...
(defun s:show-update-status (/ state message path currentVersion remoteVersion date)
  (setq state (s:get-status-value "STATE"))
  (setq message (s:get-status-value "MESSAGE"))
  (setq path (s:get-status-value "PATH"))
  (setq currentVersion (s:get-status-value "CURRENT_VERSION"))
  (setq remoteVersion (s:get-status-value "REMOTE_VERSION"))
  (setq date (s:get-status-value "DATE"))

  (if (= state "")
    (progn
      (prompt "\nAucune verification lancee.")
      (prompt "\nLance d'abord S_UPDATE_LISP.")
    )
    (progn
      (prompt "\nStatut mise a jour :")
      (prompt (strcat "\nEtat : " state))
      ...
      (cond
        ((= state "RUNNING")
          (prompt "\nLa verification est encore en cours en arriere-plan.")
        )
        ...
        ((= state "UPDATED")
          (prompt "\nLe fichier LISP a ete remplace.")
          (prompt "\nIMPORTANT : recharge le LISP avec APPLOAD ou redemarre BricsCAD.")
        )

        ((= state "ERROR")
          (prompt "\nErreur pendant la mise a jour. Tape S_UPDATE_LOG pour voir le detail.")
        )
      )
    )
  )
...
(defun c:S_UPDATE_LISP (/ psFile ok)
  (prompt "\nLancement de la verification de mise a jour en arriere-plan...")

  (s:delete-if-exists (s:update-status-file))
  (s:delete-if-exists (s:update-log-file))
  (s:delete-if-exists (s:update-version-file))
  (s:delete-if-exists (s:update-new-lisp-file))

  (setq psFile (s:create-background-update-ps))
  (setq ok (s:run-powershell-background psFile))

  (if ok
    (progn
      (prompt "\nVerification lancee. BricsCAD reste utilisable.")
      (prompt "\nTape S_UPDATE_STATUS pour voir le resultat.")
      (prompt "\nTape S_UPDATE_LOG si la mise a jour ne se fait pas.")
    )
    (prompt "\nImpossible de lancer PowerShell en arriere-plan.")
  )

  (princ)
)

(defun c:S_UPDATE_STATUS ()
  (s:show-update-status)
  (princ)
)

(defun c:S_UPDATE_LOG (/ logContent)
  (setq logContent (s:read-file-all (s:update-log-file)))
...
(defun c:S_UPDATE_RESET ()
  (s:delete-if-exists (s:update-status-file))
  (s:delete-if-exists (s:update-log-file))
  (s:delete-if-exists (s:update-version-file))
  (s:delete-if-exists (s:update-new-lisp-file))
  (prompt "\nFichiers temporaires de mise a jour supprimes.")
  (princ)
)

(defun c:S_UPDATE_INFO ()
  (prompt "\nInformations AutoUpdate :")
  (prompt (strcat "\nVersion actuelle : " *S_SCRIPT_VERSION*))
  (prompt (strcat "\nIdentifiant unique : " *S_UPDATE_UNIQUE_ID*))
  (prompt (strcat "\nURL version : " *S_UPDATE_VERSION_URL*))
  (prompt (strcat "\nURL LISP : " *S_UPDATE_LSP_URL*))

  (if *S_UPDATE_CURRENT_LSP_PATH*
    (prompt (strcat "\nChemin natif : " *S_UPDATE_CURRENT_LSP_PATH*))
    (prompt "\nChemin natif : indisponible")
  )

  (prompt (strcat "\nDossier temporaire : " (s:get-temp-folder)))
  (princ)
)
...
; ============================================================
; Cette verification ne bloque pas BricsCAD.
; Commente cette ligne si tu veux lancer uniquement avec S_UPDATE_LISP.
